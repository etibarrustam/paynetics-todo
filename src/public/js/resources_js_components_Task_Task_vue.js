...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  methods: {
    ...
    deleteTask: function deleteTask() {
      ...
      this.axios["delete"]("/api/v1/tasks/" + this.deletePostData.id).then(function (response) {
        loading.close();
    ...
    getStatuses: function getStatuses() {
      ...
      this.axios.get("/api/v1/tasks/statuses").then(function (response) {
        if (response.data.code !== 1) {
    ...
    getEmployees: function getEmployees() {
      ...
      this.axios.get("/api/v1/users/employees", {
        params: {
          project_id: this.params.project_id
        }
      }).then(function (response) {
        if (response.data.code !== 1) {
          _this8.errorNotification(response.data.validation_errors);
        }

        _this8.employees = response.data.data;
      })["catch"](function (error) {
        _this8.errorNotification(error.message);
      });
    }
  },
...
var render = function render() {
  ...
  return _c("div", [_c("div", {
  ...
  }, [_vm._v("Choose project")]), _vm._v(" "), _c("select", {
    directives: [{
      name: "model",
      rawName: "v-model",
      value: _vm.params.project_id,
      expression: "params.project_id"
    }],
    ref: "getStatus",
    ...
    on: {
      change: [function ($event) {
        ...
        _vm.$set(_vm.params, "project_id", $event.target.multiple ? $$selectedVal : $$selectedVal[0]);
      }, _vm.getTasks]
    }
    ...
    model: {
      value: _vm.task.employees,
      callback: function callback($$v) {
        _vm.$set(_vm.task, "employees", $$v);
      },
      expression: "task.employees"
    }
  })], 1), _vm._v(" "), _c("div", {
    staticClass: "form-group mt--3"
  }, [_c("label", {
  ...
  }, [_vm._v("Edit Task")])])])]), _vm._v(" "), _c("vs-dialog", {
    attrs: {
      width: "550px",
      "not-center": ""
    },
